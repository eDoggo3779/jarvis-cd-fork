...
#      bakes spack iowarp(+fuse,+redis), juicefs, redis, fio, THIS jarvis-cd
#      checkout, and clio-core at a pinned ref; its final RUN fails if any
#      required binary is missing — the install-cleanliness gate).
...
# find it by basename (container_image: "iowarp-regression-526"):
